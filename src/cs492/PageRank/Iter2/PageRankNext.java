...
//			Iterator<String> it = outLinks.iterator();
			while (st.hasMoreTokens()) {
				String value = "ToOthers" + "\t" + Double.toString(pr / tokenCount);
...
//				String[] values = iterString.split("\t");
				st = new StringTokenizer(iterString, "\t");
...
//				}
				reporter.progress();
				String toWhom = st.nextToken();
//				if (values[0].equals("ToMyself")) {
				if (toWhom.equals("ToMyself")) {
					//outlinks strings
					// 0 \t id \t numOfOutlinks \t outlinks
//					if (onlyOne != 0) {
...
//		System.out.print(sum);
		...
		JobConf conf = new JobConf(PageRankNext.class);
		conf.setJobName("PageRankNext");
